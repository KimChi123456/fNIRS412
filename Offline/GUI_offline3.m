function varargout = GUI_offline3(varargin)
% GUI_OFFLINE3 MATLAB code for GUI_offline3.fig
%      GUI_OFFLINE3, by itself, creates a new GUI_OFFLINE3 or raises the existing
%      singleton*.
%
%      H = GUI_OFFLINE3 returns the handle to a new GUI_OFFLINE3 or the handle to
%      the existing singleton*.
%
%      GUI_OFFLINE3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_OFFLINE3.M with the given input arguments.
%
%      GUI_OFFLINE3('Property','Value',...) creates a new GUI_OFFLINE3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_offline3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_offline3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_offline3

% Last Modified by GUIDE v2.5 24-Jul-2014 14:33:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_offline3_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_offline3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_offline3 is made visible.
function GUI_offline3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_offline3 (see VARARGIN)
global filename
clc
...
% Choose default command line output for GUI_offline3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_offline3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_offline3_OutputFcn(hObject, eventdata, handles) 
...
%%%%
currentfolder=pwd;
cd('D:\IU\project(filtering fNIRS)\GUI\data testNIRS');
...
set(handles.wiener_togglebutton,'visible','off');
...
elseif check_notdata==0
    ...
    data=mat2cell(data,length,[1 slit_array]);% data is a cell
    tasks=data{1,1};
    ...
    raw_data=data{1,2}; % raw_data is an array
    filter=data(:,3:end);
...
global pca_check, global wiener_check
c=4;
pca_check;
wiener_check;
www=size(oxy1)
...
if pca_check==1
    ...
    meanOxy=mean(oxy);
    OXY= oxy-repmat(meanOxy,length,1);

    meanDeoxy=mean(deoxy);
    DEOXY= deoxy-repmat(meanDeoxy,length,1);
...
if on4==0
    ...
    set(handles.wiener_togglebutton,'value',0);
    c=0;
    rest_oxy=[];
    task_oxy=[];
    rest_deoxy=[];
   ...
   %%Plot
    ...
    set(handles.wiener_togglebutton,'visible','on');
...
% --- Executes on button press in wiener_togglebutton.
function wiener_togglebutton_Callback(hObject, eventdata, handles)
% hObject    handle to wiener_togglebutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global ch,global ox1, global deox1, global ox2, global deox2
global signal,global step
...
% Hint: get(hObject,'Value') returns toggle state of wiener_togglebutton
oxy_pca=[]; deoxy_pca=[];final1=[];final2=[];
wiener_value=get(hObject,'Value');
...
if wiener_value==0
    ...
    set(handles.knum_text,'String', 'Processing...');
    %%%%%%%%%%%%%%%%%%
    K=size(rest_oxy,1);
    ...
    for CH=1:ch
       ...
       i=0;
       final=[];even=[]; odd=[]; 
       w=rest_oxy(1:K,CH);%%Resting state
       W=abs(fft(w)).^2; 
       x=oxy(i*K+1:(i+1)*K,CH);%%Signal
       X=abs(fft(x)).^2;
       Wiener=X/(X+W); %%Wiener filter
       Y=Wiener*X;
       y=ifft(sqrt(Y));%%Filtered partial signal
       Begin=y(1:end-k);
       final=Begin;
       
       for i=1:segments-1
           x=oxy((i-0.5)*K+1:(i+0.5)*K,CH);
           X=abs(fft(x)).^2;
           Wiener=X/(X+W);
           Y=Wiener*X;
           y=ifft(sqrt(Y));
       ...
       if (remains+k)>= K
           x=oxy(end-remains-k+1:end-remains-k+K,CH);
           X=abs(fft(x)).^2;
           Wiener=X/(X+W);
           Y=Wiener*X;
           y=ifft(sqrt(Y));
           add=y(k+1:end-k,1);
           final=[final; add];
%            remains=length-size(final,1)
       end
       remains=Remains;
   
       x=oxy(end-K+1:end,CH);
       X=abs(fft(x)).^2;
       Wiener=X/(X+W);
       Y=Wiener*X;
       y=ifft(sqrt(Y));
       last=y(end-remains+1:end-k);
       length_last=size(last,1);
       final=[final; last];
       final1=[final1 final];
       ...
       i=0;
       final=[];even=[]; odd=[]; 
       w=rest_deoxy(1:K,CH);%%Resting state
       W=abs(fft(w)).^2; 
       x=deoxy(i*K+1:(i+1)*K,CH);%%Signal
       X=abs(fft(x)).^2;
       Wiener=X/(X+W); %%Wiener filter
       Y=Wiener*X;
       y=ifft(sqrt(Y));%%Filtered partial signal
       ...
       if (remains+k)>= K
           x=deoxy(end-remains-k+1:end-remains-k+K,CH);
           X=abs(fft(x)).^2;
           Wiener=X/(X+W);
           Y=Wiener*X;
           y=ifft(sqrt(Y));
       ...
       assignin('base', 'final1', final1);
       assignin('base', 'oxy', oxy)
       oxy1=final1;
       deoxy1=final2;
       assignin('base','oxy1',oxy1)
       assignin('base','oxy',oxy)
    ...
    size(deoxy1)
    pause(0.75)
    set(handles.knum_text,'String', 'Done');
end
